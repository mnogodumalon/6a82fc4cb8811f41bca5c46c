...
import type { Besitzer, Hunde, Buchungen, Buchungsanfragen, Website, PfotenPortraets, CreateBesitzer, CreateHunde, CreateBuchungen, CreateBuchungsanfragen, CreateWebsite, CreatePfotenPortraets } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- PFOTEN_PORTRAETS ---
  static async getPfotenPortraets(): Promise<PfotenPortraets[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.PFOTEN_PORTRAETS}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as PfotenPortraets[];
    return hydrateRecords(records, 'pfoten_portraets');
  }
  static async getPfotenPortraet(id: string): Promise<PfotenPortraets | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.PFOTEN_PORTRAETS}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as PfotenPortraets;
    return hydrateRecords([record], 'pfoten_portraets')[0];
  }
  static async createPfotenPortraet(fields: CreatePfotenPortraets): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.PFOTEN_PORTRAETS}/records`, { fields: cleanFieldsForApi(fields as any, 'pfoten_portraets') });
    return { ...data, record_id: data.id };
  }
  static async updatePfotenPortraet(id: string, fields: Partial<CreatePfotenPortraets>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.PFOTEN_PORTRAETS}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'pfoten_portraets') });
    return { ...data, record_id: data.id };
  }
  static async deletePfotenPortraet(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.PFOTEN_PORTRAETS}/records/${id}`);
  }
