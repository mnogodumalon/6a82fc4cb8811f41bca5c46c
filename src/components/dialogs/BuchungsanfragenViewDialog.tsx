    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('buchungsanfragen', 'anfrage_status')}</Label>
            <Badge variant="secondary">{lookupLabel('buchungsanfragen', 'anfrage_status', record.fields.anfrage_status?.key) ?? record.fields.anfrage_status?.label ?? '—'}</Badge>
          </div>
