/**
 * PfotenPortraetsDialog — pre-generated create/edit dialog for PfotenPortraets.
 *
 ...
 * buchungenList (full hook array — resolves the Buchungen applookup),
 * enablePhotoScan?, enablePhotoLocation?.
 *
 * defaultValues is SHAPE-TOLERANT and its prop type is the EXPORTED
 * PfotenPortraetsDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<PfotenPortraets['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<PfotenPortraetsDialogDefaults | undefined>(undefined)
 */
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/PfotenPortraets';
...
/** Widened prefill type for PfotenPortraetsDialog.defaultValues — see file header. */
export type PfotenPortraetsDialogDefaults = PfotenPortraets['fields'];

interface PfotenPortraetsDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: PfotenPortraets['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: PfotenPortraetsDialogDefaults;
...
export function PfotenPortraetsDialog({ open, onClose, onSubmit, defaultValues, recordId, hundeList, besitzerList, buchungenList, enablePhotoScan = true, enablePhotoLocation = true }: PfotenPortraetsDialogProps) {
  const [fields, setFields] = useState<Partial<PfotenPortraets['fields']>>({});
  ...
  // Inline-Create state for "Buchungen" target. The dropdown's
  // "+ Neuer …" option opens a sub-dialog; on submit we POST, add the new
  // record to the local `extraBuchungen` list, and select it in
  // the originating Combobox via the captured `createBuchungenField`.
  const [createBuchungenOpen, setCreateBuchungenOpen] = useState(false);
  const [createBuchungenInitial, setCreateBuchungenInitial] = useState('');
  const [createBuchungenField, setCreateBuchungenField] = useState<string>('');
  const [extraBuchungen, setExtraBuchungen] = useState< Buchungen[]>([]);
  const buchungenListAll = useMemo(
    () => [...buchungenList, ...extraBuchungen],
    [buchungenList, extraBuchungen],
  );
  function openCreateBuchungen(fieldKey: string, q: string) {
    setCreateBuchungenField(fieldKey);
    setCreateBuchungenInitial(q);
    setCreateBuchungenOpen(true);
  }
  const [showErrors, setShowErrors] = useState(false);
  const REQUIRED_FIELDS = ['hund', 'besitzer', 'widmung', 'erstellungsdatum'] as const;
  ...
  }), [hundeList, besitzerList, buchungenList, ]);
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<PfotenPortraets['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'pfoten_portraets');
      await onSubmit(clean as PfotenPortraets['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="buchung" entity="Buchungen">\n${JSON.stringify(buchungenList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      const schema = `{\n  "hund": string | null, // Display name from Hunde (see <available-records>)\n  "besitzer": string | null, // Display name from Besitzer (see <available-records>)\n  "buchung": string | null, // Display name from Buchungen (see <available-records>)\n  "widmung": string | null, // Persönliche Widmung\n  "besondere_erlebnisse": string | null, // Besondere Erlebnisse während des Aufenthalts\n  "erstellungsdatum": string | null, // YYYY-MM-DD\n  "zusatztext": string | null, // Zusätzlicher Text / Anmerkungen\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["hund", "besitzer", "buchung"]);
        ...
        const buchungName = raw['buchung'] as string | null;
        if (buchungName) {
          const buchungMatch = buchungenList.find(r => matchName(buchungName!, [String(r.fields.interne_notizen ?? '')]));
          if (buchungMatch) merged['buchung'] = createRecordUrl(APP_IDS.BUCHUNGEN, buchungMatch.record_id);
        }
        return merged as Partial<PfotenPortraets['fields']>;
      });
      ...
      if (file && (file.type.startsWith('image/') || file.type === 'application/pdf')) {
        try {
          ...
          setFields(prev => ({ ...prev, hund_foto: fileUrl }));
        } catch (uploadErr) {
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('pfoten_portraets') })
    : t('new_entity', { entity: appLabel('pfoten_portraets') });
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'hund': (
      <div key="hund" className="space-y-1.5">
        ...
        <Combobox
          id="hund"
          placeholder="Welcher Hund ist das Portrait?"
    ...
    'besitzer': (
      <div key="besitzer" className="space-y-1.5">
        ...
        <Combobox
          id="besitzer"
          placeholder="Wer ist der Besitzer?"
    ...
    'buchung': (
      <div key="buchung" className="space-y-1.5">
        <Label htmlFor="buchung">{fieldLabel('pfoten_portraets', 'buchung')}</Label>
        <Combobox
          id="buchung"
          placeholder="Zu welcher Buchung gehört das?"
          items={buchungenListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.interne_notizen ?? r.record_id),
          }))}
          value={extractRecordId(fields.buchung)}
          onChange={id => setFields(f => ({ ...f, buchung: id ? createRecordUrl(APP_IDS.BUCHUNGEN, id) : undefined }))}
          onCreateNew={(q) => openCreateBuchungen("buchung", q)}
          createLabel={t('create_in', { entity: appLabel('buchungen') })}
        />
      </div>
    ),
    'widmung': (
      <div key="widmung" className="space-y-1.5">
        <Label htmlFor="widmung">{fieldLabel('pfoten_portraets', 'widmung')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Textarea
          id="widmung"
          placeholder="Persönliche Nachricht an den Besitzer, kurze Geschichte, Highlight..."
          value={fields.widmung ?? ''}
          onChange={e => setFields(f => ({ ...f, widmung: e.target.value }))}
          rows={3}
        />
        {showErrors && !fields.widmung && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'besondere_erlebnisse': (
      <div key="besondere_erlebnisse" className="space-y-1.5">
        <Label htmlFor="besondere_erlebnisse">{fieldLabel('pfoten_portraets', 'besondere_erlebnisse')}</Label>
        <Textarea
          id="besondere_erlebnisse"
          placeholder="Was ist in der Pension passiert? Highlights, lustige Momente, Besonderheiten..."
    ...
    'hund_foto': (
      <div key="hund_foto" className="space-y-1.5">
        <Label htmlFor="hund_foto">{fieldLabel('pfoten_portraets', 'hund_foto')}</Label>
        {fields.hund_foto ? (
          <div className="flex items-center gap-3 rounded-lg border p-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm truncate text-foreground">{fields.hund_foto.split("/").pop()}</p>
              <div className="flex gap-2 mt-1">
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        setFields(f => ({ ...f, hund_foto: fileUrl }));
                ...
                <button
                  ...
                  onClick={() => setFields(f => ({ ...f, hund_foto: undefined }))}
                >
                  {t('fr_remove')}
                </button>
              </div>
    ...
    'erstellungsdatum': (
      <div key="erstellungsdatum" className="space-y-1.5">
        ...
        <DatePicker
          id="erstellungsdatum"
          placeholder="Wann wurde das Portrait erstellt?"
          mode="date"
    ...
    'zusatztext': (
      <div key="zusatztext" className="space-y-1.5">
        <Label htmlFor="zusatztext">{fieldLabel('pfoten_portraets', 'zusatztext')}</Label>
        <Textarea
          id="zusatztext"
          placeholder="Weitere Informationen, Anmerkungen, Fußnote..."
  ...
  const FIELD_LABELS: Record<string, string> = {"hund": "Hund", "besitzer": "Besitzer", "buchung": "Zugehörige Buchung", "widmung": "Persönliche Widmung", "besondere_erlebnisse": "Besondere Erlebnisse während des Aufenthalts", "hund_foto": "Foto des Hundes", "erstellungsdatum": "Erstellungsdatum", "zusatztext": "Zusätzlicher Text / Anmerkungen"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"hund": {"name": "Name des Hundes", "rasse": "Rasse", "geburtsdatum": "Geburtsdatum", "geschlecht": "Geschlecht", "gewicht_kg": "Gewicht (kg)", "kastriert": "Kastriert / Sterilisiert", "impfstatus": "Impfstatus", "impfausweis_foto": "Impfausweis (Foto/Dokument)", "fuetterungshinweise": "Fütterungshinweise", "medikamente": "Medikamente & Besonderheiten", "tierarzt_name": "Tierarzt (Name & Praxis)", "tierarzt_telefon": "Tierarzt Telefon", "besitzer": "Besitzer", "foto": "Foto des Hundes"}, "besitzer": {"vorname": "Vorname", "nachname": "Nachname", "telefon": "Telefonnummer", "email": "E-Mail-Adresse", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "notizen": "Notizen"}, "buchung": {"hund": "Hund", "besitzer": "Besitzer", "anreise": "Anreisedatum", "abreise": "Abreisedatum", "platz": "Platznummer", "status": "Status", "preis_gesamt": "Gesamtpreis (€)", "zahlungsstatus": "Zahlungsstatus", "interne_notizen": "Interne Notizen"}};
  ...
  return (
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.PFOTEN_PORTRAETS} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
    ...
    {createBuchungenOpen && (
      <BuchungenDialog
        open={createBuchungenOpen}
        onClose={() => setCreateBuchungenOpen(false)}
        onSubmit={async (newFields) => {
          const result = await LivingAppsService.createBuchungenEntry(newFields as any) as { id?: string };
          if (result?.id) {
            const newRec = { record_id: result.id, fields: newFields } as unknown as Buchungen;
            setExtraBuchungen(prev => [...prev, newRec]);
            const url = createRecordUrl(APP_IDS.BUCHUNGEN, result.id);
            setFields(prev => ({ ...prev, [createBuchungenField]: url } as any));
          }
          setCreateBuchungenOpen(false);
        }}
        defaultValues={createBuchungenInitial
          ? ({ interne_notizen: createBuchungenInitial } as any)
          : undefined}
        hundeList={hundeList}
        besitzerList={besitzerList}
      />
