...
import type { Buchungen, Hunde, Besitzer, LookupValue } from '@/types/app';
...
export type BuchungenDialogDefaults = Omit<Buchungen['fields'], 'platz' | 'status' | 'zahlungsstatus'> & {
    platz?: LookupValue | string;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  platz: LOOKUP_OPTIONS['buchungen']?.['platz'] ?? [],
  status: LOOKUP_OPTIONS['buchungen']?.['status'] ?? [],
  zahlungsstatus: LOOKUP_OPTIONS['buchungen']?.['zahlungsstatus'] ?? [],
};
...
export function BuchungenDialog({ open, onClose, onSubmit, defaultValues, recordId, hundeList, besitzerList, enablePhotoScan = true, enablePhotoLocation = true }: BuchungenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['hund', 'besitzer', 'anreise', 'abreise', 'platz', 'status'] as const;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'platz': (
      <div key="platz" className="space-y-1.5">
        <Label htmlFor="platz">{fieldLabel('buchungen', 'platz')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Select
          value={lookupKey(fields.platz) ?? ''}
          onValueChange={v => setFields(f => ({ ...f, platz: v === 'none' ? undefined : v as any }))}
        >
          <SelectTrigger id="platz" className="max-sm:h-11"><SelectValue placeholder="z. B. Platz 1, Platz 2" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="none">—</SelectItem>
            <SelectItem value="platz_1">{lookupLabel('buchungen', 'platz', 'platz_1') ?? 'Platz 1'}</SelectItem>
            <SelectItem value="platz_2">{lookupLabel('buchungen', 'platz', 'platz_2') ?? 'Platz 2'}</SelectItem>
            <SelectItem value="platz_3">{lookupLabel('buchungen', 'platz', 'platz_3') ?? 'Platz 3'}</SelectItem>
            <SelectItem value="platz_4">{lookupLabel('buchungen', 'platz', 'platz_4') ?? 'Platz 4'}</SelectItem>
            <SelectItem value="platz_5">{lookupLabel('buchungen', 'platz', 'platz_5') ?? 'Platz 5'}</SelectItem>
            <SelectItem value="platz_6">{lookupLabel('buchungen', 'platz', 'platz_6') ?? 'Platz 6'}</SelectItem>
            <SelectItem value="platz_7">{lookupLabel('buchungen', 'platz', 'platz_7') ?? 'Platz 7'}</SelectItem>
            <SelectItem value="platz_8">{lookupLabel('buchungen', 'platz', 'platz_8') ?? 'Platz 8'}</SelectItem>
            <SelectItem value="platz_9">{lookupLabel('buchungen', 'platz', 'platz_9') ?? 'Platz 9'}</SelectItem>
            <SelectItem value="platz_10">{lookupLabel('buchungen', 'platz', 'platz_10') ?? 'Platz 10'}</SelectItem>
            <SelectItem value="platz_11">{lookupLabel('buchungen', 'platz', 'platz_11') ?? 'Platz 11'}</SelectItem>
            <SelectItem value="platz_12">{lookupLabel('buchungen', 'platz', 'platz_12') ?? 'Platz 12'}</SelectItem>
          </SelectContent>
        </Select>
        {showErrors && !fields.platz && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'status': (
      <div key="status" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('buchungen', 'status', 'geplant') ?? 'Geplant'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('buchungen', 'status', 'anwesend') ?? 'Anwesend'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('buchungen', 'status', 'abgereist') ?? 'Abgereist'}
          </button>
          <button
            type="button"
            role="radio"
    ...
    'preis_gesamt': (
      <div key="preis_gesamt" className="space-y-1.5">
        <Label htmlFor="preis_gesamt">{fieldLabel('buchungen', 'preis_gesamt')}</Label>
        <Input
          id="preis_gesamt"
          type="number"
          step="any"
    ...
    'zahlungsstatus': (
      <div key="zahlungsstatus" className="space-y-1.5">
        <Label htmlFor="zahlungsstatus">{fieldLabel('buchungen', 'zahlungsstatus')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('buchungen', 'zahlungsstatus', 'offen') ?? 'Offen'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.zahlungsstatus) === 'teilweise'}
            onClick={() => setFields(f => ({ ...f, zahlungsstatus: (lookupKey(f.zahlungsstatus) === 'teilweise' ? undefined : 'teilweise') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.zahlungsstatus) === 'teilweise'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('buchungen', 'zahlungsstatus', 'teilweise') ?? 'Teilweise bezahlt'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('buchungen', 'zahlungsstatus', 'bezahlt') ?? 'Bezahlt'}
          </button>
        </div>
      </div>
    ),
    'interne_notizen': (
      <div key="interne_notizen" className="space-y-1.5">
        ...
        <Textarea
          id="interne_notizen"
          placeholder="Spezielle Anforderungen, Gebühren, besondere Regelungen..."
  ...
  const FIELD_LABELS: Record<string, string> = {"hund": "Hund", "besitzer": "Besitzer", "anreise": "Anreisedatum", "abreise": "Abreisedatum", "platz": "Platznummer", "status": "Status", "preis_gesamt": "Gesamtpreis (€)", "zahlungsstatus": "Zahlungsstatus", "interne_notizen": "Interne Notizen"};
  const CURRENCY_KEYS = new Set<string>(["preis_gesamt"]);
